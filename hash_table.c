...
void *hash_table_add(HashTable ht, void *item)
{
	...
	for (int i = 0; ; i++) {
		uint32_t key = get_key_from_item(item);
		if (key == 0) {
			FATAL_ERROR("Bad key");
		}
		void *value = find_slot_table1(ht, key);
...
// static void hash_remove(uint32_t key)
// {
